...
class Rule:
    def build_regexp(self, rules: Dict[int, 'Rule']) -> str:
        pass
...
class SeqRule(Rule):
    ...
    def build_regexp(self, rules: Dict[int, Rule]) -> str:
        return ''.join(rules[rule_id].build_regexp(rules)
...
class AltRule(Rule):
    ...
    def build_regexp(self, rules: Dict[int, Rule]) -> str:
        return '(%s)' % (
            '|'.join(alt.build_regexp(rules) for alt in self.alternatives)
        )


def build_validator(rules):
    return re.compile('^' + rules[0].build_regexp(rules) + '$').match
...
with open("input" if len(sys.argv) < 2 else sys.argv[1]) as f:
    ...
    valid_message = build_validator(rules)
    n = 0
    for line in f:
        line = line.strip()

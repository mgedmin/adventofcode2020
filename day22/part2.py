...
with open("input" if len(sys.argv) < 2 else sys.argv[1]) as f:
    ...
    player1 = read_cards(f)
    line = next(f)
    assert line == "Player 2:\n"
    player2 = read_cards(f)


def play_game(player1, player2, game_counter=None):
    seen = set()
    ...
    while player1 and player2:
        ...
        if (player1, player2) in seen:
            ...
            return player1, []  # player 1 wins the game
        ...
        if len(player1) >= 1 + top1 and len(player2) >= 1 + top2:
            # recursive play!
            if '-v' in sys.argv:
                print('Playing a sub-game to determine the winner...\n')
            player1_wins = play_game(
                player1[1:1+top1], player2[1:1+top2], game_counter)[0]
            if '-v' in sys.argv:
                print(f'\n...anyway, back to game {game}.')
        else:
            player1_wins = top1 > top2
        if player1_wins:
            if '-v' in sys.argv:
                print(f'Player 1 wins round {round} of game {game}!')
            player1 = player1[1:] + (top1, top2)
            player2 = player2[1:]
        else:
            if '-v' in sys.argv:
                print(f'Player 2 wins round {round} of game {game}!')
            player1 = player1[1:]
            player2 = player2[1:] + (top2, top1)
        round += 1
    if '-v' in sys.argv:
